...
foreach ($tradesPage->getTrades()->toArray() as $trade) {
    echo $trade->getBaseAmount() . " at " . $trade->getPrice() . "\n";
}
```
...
$balancesPage = $sdk->claimableBalances()
    ->forAsset("USD:GISSUER...")
    ->execute();

foreach ($balancesPage->getClaimableBalances()->toArray() as $balance) {
    echo $balance->getAmount() . " " . $balance->getAsset() . "\n";
}
```
...
$usdAsset = Asset::createNonNativeAsset("USD", "GISSUER...");
$sdk->trades()->forAssetPair(Asset::native(), $usdAsset)->cursor("now")->stream(function(TradeResponse $trade) {
    echo "XLM/USD trade at " . $trade->getPrice() . "\n";
});
```
...
$sdk->orderBook()
    ...
    ->stream(function(OrderBookResponse $orderBook) {
        echo "Bids: " . count($orderBook->getBids()) . "\n";
        echo "Asks: " . count($orderBook->getAsks()) . "\n";
    });
```
...
<?php
use Soneso\StellarSDK\StellarSDK;
use Soneso\StellarSDK\Responses\Offer\OfferResponse;

$sdk = StellarSDK::getTestNetInstance();

$sdk->offers()->forAccount("GABC...")->cursor("now")->stream(function(OfferResponse $offer) {
    echo "Offer " . $offer->getId() . ": " . $offer->getAmount() . " at " . $offer->getPrice() . "\n";
});
```
...
$sdk->accounts()->forAccount("GABC...")->cursor("now")->stream(function(AccountResponse $account) {
